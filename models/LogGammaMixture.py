...
class LogGammaMixtureSemiVAE(LogGammaSemiVAE):

    def __init__(self,params,paramFile=None,reloadFile=None):
        super(LogGammaMixtureSemiVAE,self).__init__(params,paramFile=paramFile,reloadFile=reloadFile)
    def _createParams(self):
        npWeights = super(LogGammaMixtureSemiVAE,self)._createParams()
        K = self.params['nclasses']
        npWeights['logbetaprior_W'] = np.zeros((K,K)).astype(config.floatX)
        if self.params['learn_posterior']:
            npWeights['logbeta_y_W'] = np.eye(K).astype(config.floatX)
        return npWeights
    ...
    def _variationalDirichletMixture(self,beta,betaprior,qy_x):
        ...
        for y in range(K):
            ...
            beta_q_ = T.sum(Y.reshape(list(Y.shape)+[1])*betaprior_W,axis=1)
            if self.params['learn_posterior']:
                beta_y_W = T.exp(self.tWeights['logbeta_y_W'].reshape((1,K,K)))
                beta_q_y = beta_q + T.sum(Y.reshape(list(Y.shape)+[1])*beta_y_W,axis=1)
            else:
                beta_q_y = beta_q + Y
    ...
    def _variationalDirichlet(self,beta_q,beta_p):
        K = self.params['nclasses']
        KL_alpha = self._KL_Loggamma(beta_q,beta_p)
        loggamma_variates = theano.gradient.disconnected_grad(self.rng_loggamma(beta_q))
    ...
    def _buildGraph(self, X, eps, betaprior=0.2, Y=None, dropout_prob=0.,add_noise=False,annealKL=None,evaluation=False,modifiedBatchNorm=False,graphprefix=None):
        ...
        if Y is None:
            ...
            if self.params['learn_posterior']:
                beta_y_W = T.exp(self.tWeights['logbeta_y_W'].reshape((1,K,K)))
                beta_q_y = beta + T.sum(Y.reshape(list(Y.shape)+[1])*beta_y_W,axis=1)
                U, KL_alpha = self._variationalDirichlet(beta_q_y,beta_p)
            else:
                beta_q_y = beta + Y
                U, KL_alpha = self._variationalDirichlet(beta_q_y,beta_p)
            beta = beta_q_y
