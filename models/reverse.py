...
class LogGammaSemiVAEReverse(LogGammaSemiVAE, object):
    def __init__(self, params, paramFile=None, reloadFile=None):
        self._tVariables = {}
        self._BNprefix = None
        super(LogGammaSemiVAEReverse,self).__init__(params, paramFile=paramFile, reloadFile=reloadFile)
    ...
    def _createParams(self):
        ...
        for a_l in range(self.params['alpha_inference_layers']):
            ...
            if self.params['nonlinearity']=='maxout':
                dim_output= DIM_HIDDEN*self.params['maxout_stride']
            # log(beta)
            if a_l == 0 and not self.params['bilinear']:
        ...
        if self.params['inference_model']=='single':
            ...
            if self.params['nonlinearity']=='maxout':
                dim_output= DIM_HIDDEN*self.params['maxout_stride']
            npWeights['q_alpha_Z_W'] = self._getWeight((self.params['dim_stochastic'],DIM_HIDDEN))
            npWeights['q_alpha_Z_b'] = self._getWeight((DIM_HIDDEN,))
    ...
    def _buildClassifierEvaluation(self, hx, mu, logcov2, Y, S=30,graphprefix=None):
        probs = 0
        for s in range(S):
            Z = mu + T.exp(0.5*logcov2)*self.srng.normal(mu.shape,0,1,dtype=config.floatX)
            logbeta = self._build_logbeta(hx, Z, evaluation=True,graphprefix=graphprefix)
            probs = T.nnet.softmax(logbeta) + probs
            #T.nnet.categorical_crossentropy returns a vector of length batch_size
        probs = probs/float(S)
        loss= T.nnet.categorical_crossentropy(probs,Y) 
        ncorrect = T.eq(T.argmax(probs,axis=1),Y).sum()
    ...
    def _build_logbeta(self, hx, Z, dropout_prob=0.,evaluation=False,graphprefix=None):
        self._BNprefix=graphprefix
        self._p(('Inference with dropout :%.4f')%(dropout_prob))
        hx = self._dropout(hx,dropout_prob)
        Z_embed = self._LinearNL(W=self.tWeights['q_alpha_Z_W'],b=self.tWeights['q_alpha_Z_b'],inp=Z,onlyLinear=True)
        inp = T.concatenate([hx,Z_embed],axis=1)
        logbeta = self._buildHiddenLayers(inp,self.params['alpha_inference_layers'],'q_logbeta_{layer}',evaluation)
        logbeta = self._LinearNL(W=self.tWeights['q_logbeta_out_W'],b=self.tWeights['q_logbeta_out_b'],inp=logbeta,onlyLinear=True)
        logbeta = T.clip(logbeta,-7,7)
        return logbeta

    def _build_hx(self, X, dropout_prob=0.,evaluation=False,graphprefix=None):
        self._BNprefix=graphprefix
        self._p(('Inference with dropout :%.4f')%(dropout_prob))
    ...
    def _build_qz(self,hx,evaluation,graphprefix):
        if self.params['dropout_hx']>0 and evaluation==False:
            hx = self._dropout(hx,self.params['dropout_hx'])
        self._BNprefix=graphprefix
        #infer mu and logcov
        q_Z_hidden = self._buildHiddenLayers(hx,self.params['z_inference_layers'],'q_Z_{layer}',evaluation)
    ...
    def _buildGraph(self, X, eps, betaprior=0.2, Y=None, dropout_prob=0.,add_noise=False,annealKL=None,evaluation=False,graphprefix=None):
        """
        ...
        hx= self._build_hx(X,dropout_prob,evaluation,graphprefix)
        self._addVariable(graphprefix+'_hx'+suffix,hx,ignore_warnings=True)

        # q(z|x,alpha)
        mu, logcov = self._build_qz(hx,evaluation,graphprefix) 
        ...
        logbeta = self._build_logbeta(hx, Z, evaluation=evaluation,graphprefix=graphprefix)
        beta = T.exp(logbeta)
        if Y!=None:
        ...
        #generative model
        paramsX, nllX = self._buildEmission(alpha, Z, X, graphprefix, evaluation=evaluation)
        ...
        if evaluation == False:
            if Y==None or self.params['logpxy_discrete']==False:
                ...
                nllX = nllX.reshape([-1])
                f = theano.gradient.disconnected_grad(nllX)
    ...
    def _getModelOutputs(self,outputsU,outputsL,suffix=''):
        my_outputs = {
                         'KL_loggammaU':outputsU['KL_loggamma'],
                         ...
                         'Y':outputsL['Y'],
                         'KL_loggammaL':outputsL['KL_loggamma'],
    ...
    def _buildModel(self):
        ...
        hx_e = self._tVariables[graphprefixL+'_hx_e']
        mu_e = self._tVariables[graphprefixL+'_mu_e']
        logcov2_e = self._tVariables[graphprefixL+'_logcov2_e']
        _,crossentropyloss_e,ncorrect_e = self._buildClassifierEvaluation(hx_e,mu_e,logcov2_e,Y,S=30,graphprefix=graphprefixL)
        ...
        self.updates += optimizer_up+annealKL_update+annealCW_update+ctr_update+annealBP_update
...
#        self.reset_anneal=theano.function([],anneal.sum(), updates = [(anneal,0.01)], name='reset anneal')
    def sample(self,onehot=True,nsamples=100):
        ...
        if onehot:
            ...
            u = np.vectorize(randomLogGamma)(self.params['betaprior']*np.ones((nsamples,K)))
            alpha = np.exp(u-logsumexp(u,axis=1,keepdims=True)).astype(config.floatX)
            return {'U':self.reconstructU(z,alpha),'L':self.reconstructL(z,alpha)}
    ...
    def learn(self, dataset, epoch_start=0, epoch_end=1000, batch_size=200, shuffle=True, 
        ...
        #normalize = ['bound','cost','accuracy','boundU','boundL','classification_loss']
                ...
                if debug:
                    ...
                    for k,v in batch_outputs.iteritems():
                        if k not in epoch_outputs or v.size > 1:
                            ...
                            epoch_outputs[k] += v 
                    epoch_outputs['YU']=dataset['YU'][idxlist[st_idx:end_idx]]
...
#            classifier /= float(N)
            ...
            if evalfreq is not None and epoch%evalfreq==0:
                ...
                samples_onehot = self.sample(onehot=True,nsamples=100)
                samples_dirichlet = self.sample(onehot=False,nsamples=100)
                samples = {'onehotU':samples_onehot['U'],
                           'onehotL':samples_onehot['L'],
                           'dirichletU':samples_dirichlet['U'],
                           'dirichletL':samples_dirichlet['L'],
                          }
                for k,v in samples.iteritems():
                    v = np.expand_dims(v,axis=0)
                    if k not in track_samples:
                        track_samples[k] = v 
                    else:
                        track_samples[k] = np.concatenate([track_samples[k],v],axis=0)
                if 'epochs' not in track_samples:
                    track_samples['epochs'] = np.array(epoch)
                else:
                    track_samples['epochs'] = np.hstack([track_samples['epochs'],epoch])
            ...
            if savefreq is not None and epoch%savefreq==0:
                ...
                saveHDF5(os.path.join(savedir,'samples.h5'), track_samples)
        return save_map, track_samples
