...
class LogGammaSemiVAE(BaseModel, object):
    def __init__(self, params, paramFile=None, reloadFile=None):
        self._tVariables = {}
        self._BNprefix = None
        super(LogGammaSemiVAE,self).__init__(params, paramFile=paramFile, reloadFile=reloadFile)
    ...
    def _buildGraph(self, X, eps, betaprior=0.2, Y=None, dropout_prob=0.,add_noise=False,annealKL_Z=None,annealKL_alpha=None,evaluation=False,modifiedBatchNorm=False,graphprefix=None):
        ...
        if Y!=None:
            if self.params['logpxy_discrete']:
                ...
                if self.params['learn_posterior']:
                    beta += Y*T.nnet.softplus(self.tWeights['posterior_W'][0]) 
                else:
                    beta += Y
        ...
        if evaluation:
            ...
            if annealKL_Z == None and annealKL_alpha == None:
                objfunc = bound 
            elif annealKL_Z == None:
                objfunc = annealKL_alpha*KL_loggamma.sum() + KL_Z.sum() + NLL
            elif annealKL_alpha == None:
                objfunc = KL_loggamma.sum() + annealKL_Z*KL_Z.sum() + NLL
            else:
                objfunc = annealKL_alpha*KL_loggamma.sum() + annealKL_Z*KL_Z.sum() + NLL
        ...
        if evaluation == False:
            if Y==None or self.params['logpxy_discrete']==False:
                ...
                if self.params['negKL']:
                    ...
                    if annealKL_Z!=None:
                        f = theano.gradient.disconnected_grad(annealKL_Z*KL_Z+nllX)
                    else:
    ...
    def _buildModel(self):
        ...
        trainobjective = annealBound*(objfuncU_t.sum() + self.params['boundXY_weight']*objfuncL_t.sum()) + annealCW*self.params['classifier_weight']*crossentropyloss_t.sum() 
    ...
    def learn(self, dataset, epoch_start=0, epoch_end=1000, batch_size=200, shuffle=True, 
        ...
        for epoch in range(epoch_start, epoch_end+1):
            ...
            for bnum,st_idx in enumerate(range(0,N,batch_size)):
                ...
                minibatch = {
                             'XU':self.sampleDataset(dataset['XU'][idxlist[st_idx:end_idx]]),
                             'XL':self.sampleDataset(dataset['XL'][idx_labeled]),
                             'Y':dataset['YL'][idx_labeled],
                            }
                if self.params['num_samples'] > 1:
                    for k,v in minibatch.iteritems():
                        if k=='Y':
                            minibatch[k] = np.tile(v,reps=self.params['num_samples'])
                        else:
                            minibatch[k] = np.tile(v,reps=(self.params['num_samples'],1))
                    minibatch['epsU'] = np.random.randn(Nbatch*self.params['num_samples'],self.params['dim_stochastic']).astype(config.floatX)
                    minibatch['epsL'] = np.random.randn(Nbatch*self.params['num_samples'],self.params['dim_stochastic']).astype(config.floatX)
                else:
                    minibatch['epsU'] = np.random.randn(Nbatch,self.params['dim_stochastic']).astype(config.floatX)
                    minibatch['epsL'] = np.random.randn(Nbatch,self.params['dim_stochastic']).astype(config.floatX)
                ...
                if debug:
                    ...
                    for k,v in batch_outputs.iteritems():
                        if k not in epoch_outputs or v.size > 1:
                            if v.size > 1:
                                epoch_outputs[k] = v[:Nbatch]
                            else:
                                epoch_outputs[k] = v 
                        elif k in not_normalized:
                            if v.size > 1:
                                epoch_outputs[k] = np.hstack([epoch_outputs[k],v[:Nbatch]])
                            else:
                                epoch_outputs[k] = np.hstack([epoch_outputs[k],v])
                        else:
                            epoch_outputs[k] += v 
                    epoch_outputs['YL']=dataset['YL'][idx_labeled]
...
#            classifier /= float(N)
            ...
            for k,v in sorted(epoch_outputs.iteritems(),key=lambda x:x[0]):
                if v.size <= 1 and 'YU' != k:
                    epoch_outputs[k] = v/float(N*self.params['num_samples'])
