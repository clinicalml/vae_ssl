...
class LogisticNormalSemiVAE(LogGammaSemiVAE):
    ...
    def _createParams(self):
        ...
        if self.params['inference_model']=='single':
            ...
            dim_input = K
            for yl in range(self.params['y_inference_layers']):
                dim_output = DIM_HIDDEN
                if self.params['nonlinearity']=='maxout':
                    dim_output= dim_output*self.params['maxout_stride']
                npWeights['q_Z2_mu|y_%s_W'%yl] = self._getWeight((dim_input, dim_output)) 
                npWeights['q_Z2_mu|y_%s_b'%yl] = self._getWeight((dim_output, )) 
                dim_input = DIM_HIDDEN
            dim_output = K 
            npWeights['q_Z2_mu|y_W'] = self._getWeight((dim_input, dim_output))
            npWeights['q_Z2_logcov2|y_W'] = self._getWeight((dim_input, dim_output))
            if self.params['learn_posterior']:
                npWeights['q_Z2_mu|y_b'] = self._getWeight((dim_output, ))
                npWeights['q_Z2_logcov2|y_b'] = self._getWeight((dim_output, ))
        else:
    ...
    def _buildInferenceY(self, Y, evaluation=False,graphprefix=None):
        if self.params['separateBNrunningstats']:
            self._BNprefix=graphprefix
        else:
            self._BNprefix=None
        hy = self._buildHiddenLayers(Y,self.params['y_inference_layers'],'q_Z2_mu|y_{layer}',evaluation)
        return hy
    ...
    def _productGaussians(self,mu1,mu2,s1,s2):
        # s=log(sigma^2)
        # sigma^2 = exp(s)
        # sig=sigma^2
        sig1 = T.exp(s1)
        sig2 = T.exp(s2)
        mu12 = (mu1*sig2 + mu2*sig1)/(sig1+sig2)
        sig12 = sig1*sig2/(sig1+sig2)
        s12 = T.log(sig12)
        return mu12, s12

    def _getWeightsQY(self,Y):
        K = self.params['nclasses']
        mu = []
        logcov2 = []
        for y in range(K):
            mu.append(self.tWeights['q_Z2_mu_W|%s'%y].reshape((1,-1)))
            logcov2.append(self.tWeights['q_Z2_logcov2_W|%s'%y].reshape((1,-1)))
        mu = T.concatenate(mu,axis=0)
        logcov2 = T.concatenate(logcov2,axis=0)
        mu = T.dot(Y,mu)
        logcov2 = T.dot(Y,logcov2)
        return mu, logcov2
    ...
    def _buildGraph(self, X, eps, betaprior=0.2, Y=None, dropout_prob=0.,add_noise=False,annealKL=None,evaluation=False,modifiedBatchNorm=False,graphprefix=None):
        ...
        if Y!=None:
            hy = self._buildInferenceY(Y,evaluation,graphprefix)
            if self.params['learn_posterior']:
                mu_Z2_y = self._LinearNL(W=self.tWeights['q_Z2_mu|y_W'],b=self.tWeights['q_Z2_mu|y_b'],inp=hy,onlyLinear=True)
                logcov2_Z2_y = self._LinearNL(W=self.tWeights['q_Z2_logcov2|y_W'],b=self.tWeights['q_Z2_logcov2|y_b'],inp=hy,onlyLinear=True)
            else:
                mu_Z2_y = self._LinearNL(W=self.tWeights['q_Z2_mu|y_W'],b=Y,inp=hy,onlyLinear=True)
                logcov2_Z2_y = self._LinearNL(W=self.tWeights['q_Z2_logcov2|y_W'],b=Y,inp=hy,onlyLinear=True)
            mu_Z2, logcov2_Z2 = self._productGaussians(mu_Z2,mu_Z2_y,logcov2_Z2,logcov2_Z2_y)
...
#        self.reset_anneal=theano.function([],anneal.sum(), updates = [(anneal,0.01)], name='reset anneal')
    ...
    def sample(self,onehot=True,nsamples=100):
        ...
        if onehot:
            ...
            z = np.concatenate((np.array([(i,j) for i in range(-5,5,1) for j in range(-5,5,1)]),np.zeros((100,self.params['dim_stochastic']-2))),axis=-1).astype(config.floatX)
            ...
            print alpha.shape
            assert(z.shape==alpha.shape,'shape mismatch')
            return {'U':self.reconstructU(z,alpha),'L':self.reconstructL(z,alpha)}
