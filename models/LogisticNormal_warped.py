...
class LogisticNormal(LogGammaSemiVAE):

    def __init__(self,params,paramFile=None,reloadFile=None):
        super(LogisticNormal,self).__init__(params,paramFile=paramFile,reloadFile=reloadFile)

    def _sample_Y(self,g):
        if self.params['model']=='LogisticNormal_warped':
            y = T.nnet.softmax(g*self.tWeights['sharpening'])
        elif self.params['model']=='STLogisticNormal_warped':
            y = T.nnet.softmax(g*self.tWeights['sharpening'])
            ...
            assert False, 'unhandled LogisticNormal type %s' % self.params['model']
        return y
    ...
    def _buildGraph(self, X, eps, betaprior=0.2, Y=None, dropout_prob=0.,add_noise=False,annealKL_Z=None,annealKL_alpha=None,evaluation=False,modifiedBatchNorm=False,graphprefix=None):
        ...
        Z2, KL_Z2 = self._variationalGaussian(mu_Z2,logcov=0,eps=eps2)
